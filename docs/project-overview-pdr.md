...
- **F3.6** Auto preload AI detection when text protection enabled (v1.1.23)
- **F3.7** Sensitivity adjustment slider for artifact detection
...
- **F4.8** Sliding window preview (10-page window ±5 from current) for memory efficiency (v1.1.23)
...
- **N1.5** Sliding window preview limits RAM to 10 pages (v1.1.23)
- **N1.6** Background file preload for batch processing (v1.1.23)
- **N1.7** ONNX inference optimization with hardware acceleration support
...
- **N4.2** Keyboard shortcuts (Ctrl+Z/Cmd+Z undo, Cmd+A/Alt+A draw, Cmd+S/Alt+S select, +/- zoom, [ ] batch nav)
- **N4.3** Contextual help and tooltips
- **N4.4** Lazy-loaded page counts in batch sidebar (v1.1.23)
- **N4.5** Multi-language support (Vietnamese/English)
...
- Render 10 trang một (sliding window mode) - Only keeps 10 pages in RAM (current page ±5)
- AI detection preload - Auto preload AI detection when text protection enabled
- Background file preload - Preload next file in batch mode
- Lazy loading page counts - Load page counts in batches in sidebar
- Keyboard shortcuts [ ] - Navigate prev/next file in batch mode
- Fixed TextProtectionOptions dataclass access
- Fixed _load_from_cache invalid method call
- Fixed centering for both Gốc and Đích panels with different sizes
