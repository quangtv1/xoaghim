...
| `sidebar_advanced_filter.py` | 245 | Collapsible filter panel (size/orientation/range) |
| `page_metadata_loader.py` | 98 | Lazy page metadata loading (ThreadPoolExecutor) |
...
#### sidebar_advanced_filter.py (245 LOC)
- **SidebarAdvancedFilter** - Collapsible advanced filter panel
  - Filter by page size (A0-A7) with checkboxes
  - Filter by orientation (landscape/portrait)
  - Filter by page range (e.g., "1-5, 7, 10-15")
  - Validation for range syntax
  - Signals: `apply_requested`, `reset_requested`
  - Collapse/expand with visual chevron indicator

#### page_metadata_loader.py (98 LOC)
- **PageMetadataLoader** - Async page metadata loader
  - Detects page size from dimensions (A0-A7 with ±15pt tolerance)
  - Detects orientation (landscape/portrait) from aspect ratio
  - ThreadPoolExecutor batch loading (10 files parallel)
  - Signals: `metadata_loaded`, `batch_complete`
- **Helper Functions:**
  - `detect_page_size(w, h)` - Detect A-series size category
  - `load_file_metadata(path)` - Load all pages' metadata
...
### v1.1.24 (Current - 2026-03-16)
#### Advanced Sidebar Filter for Page Selection
- **Files:** `sidebar_advanced_filter.py` (245 LOC), `page_metadata_loader.py` (98 LOC)
- **Features:**
  - Collapsible filter panel at bottom of sidebar
  - Filter draw/clean operations by:
    - Page size (A0-A7 categories)
    - Orientation (landscape/portrait)
    - Page range (1-5, 7, 10-15 syntax)
  - Apply/Reset buttons with validation
  - Metadata loading: async ThreadPoolExecutor (10 files parallel, batch size 10)
  - Page dimensions tolerance: ±15 points for size detection
  - Only affects targeted operations, not entire batch

### v1.1.23 (Previous - 2026-01-27)
